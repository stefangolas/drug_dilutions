...
agpumps_path = os.path.join(methods_dir, 'perma_pump', 'agrow_pumps')
...
#for imp_path in (reader_mod_path, pump_pkg_path, shaker_pkg_path, agpumps_path):
...
#import pyhamilton
from pyhamilton import (HamiltonInterface, LayoutManager, ResourceType, Plate24, Plate96, Tip96, Plate384,
...
def lid_rp_to_park(ham_int, rp_seq, park_seq, lid_height = 5):
    move_by_seq(ham_int, rp_seq, park_seq, lid_height)

def lid_park_to_rp(ham_int, park_seq, rp_seq, lid_height = 5):
    move_by_seq(ham_int,  park_seq, rp_seq, lid_height)
...
def write_state_data(state):
    filename = 'method_local_'+ROBOID+'//state_data.pickle'
...
def overwrite_state_data(idx, value):
    filename = 'method_local_'+ROBOID+'//state_data.pickle'
    infile = open(filename,'rb')
    state_list = list(pickle.load(infile))
    state_list[idx] = value
    infile.close()
    outfile = open(filename,'wb')
    state_tuple = tuple(state_list)
    pickle.dump(state_tuple, outfile)
    outfile.close()
...
def _return_barcode(CSBarcode):
    global scannedBarcode
    scannedBarcode=CSBarcode.Data
    
def read_barcode(ham, source_plate, target_plate, barcode_plate_1, barcode_plate_2, try_inversions=None):

    scannerTypes=[clr.CSProxy.ScannerTypes.IBMHID]
    _scannerTypes = Array[Int16](scannerTypes)

    _scannerList=List[clr.CSProxy.Scanner](0)
    

    status,curScanners=clr.CSProxy.CoreScannerProxy.Instance.OpenAndGetScanners(_scannerTypes,_scannerList)
    

    
    logging.info('move_plate: Moving plate and reading barcode ' + source_plate.layout_name() + ' to ' + target_plate.layout_name())
    src_pos = labware_pos_str(source_plate, 0)
    trgt_pos = labware_pos_str(target_plate, 0)
    bar_pos_1 = labware_pos_str(barcode_plate_1, 0)
    bar_pos_2 = labware_pos_str(barcode_plate_2, 0)
    if try_inversions is None:
        try_inversions = (0, 1)
    for inv in try_inversions:
        cid = ham.send_command(ISWAP_GET, plateLabwarePositions=src_pos, gripHeight=6, inverseGrip=1, widthBefore=133)
        try:
        ...
        raise IOError
    if len(list(curScanners))>0:
        
        clr.CSProxy.CoreScannerProxy.Instance.BarcodeEvent += clr.CSProxy.BarcodeEventHandler(_return_barcode)
        curScanners[0].ScanEnable()

        
        cid= ham.send_command(ISWAP_MOVE,plateLabwarePositions=bar_pos_1)
        try:
            ham.wait_on_response(cid, raise_first_exception=True, timeout=60)
        except PositionError:
            raise IOError
        print(scannedBarcode)    
        cid= ham.send_command(ISWAP_MOVE,plateLabwarePositions=bar_pos_2)
        try:
            ham.wait_on_response(cid, raise_first_exception=True, timeout=60)
        except PositionError:
            raise IOError
...
#        try_inversions = (0, 1)
#    for inv in try_inversions:
#        cid = ham.send_command(ISWAP_GET, plateLabwarePositions=src_pos, gripHeight=gripHeight, inverseGrip=inv, widthBefore=135)
#        try:
...
def move_by_seq(ham, source_plate_seq, target_plate_seq, grip_height = 0, try_inversions=None):
    ...
    if try_inversions is None:
        try_inversions = (0, 1)
    for inv in try_inversions:
        cid = ham.send_command(ISWAP_GET, plateSequence=source_plate_seq, gripHeight=grip_height, gripForce=2, inverseGrip=inv,transportMode=0, widthBefore = 132)
        try:
            ham.wait_on_response(cid, raise_first_exception=True, timeout=120)
            break
        except PositionError:
...
def read_plate(ham_int, reader_int, reader_site, plate, protocol_names, plate_id=None, async_task=None, plate_destination=None, using_lid = False):
    ...
    print("Trying to create interface")
    ...
    print("Trying to move plate")
    move_plate(ham_int, plate, reader_site, gripHeight = gripHeight)
    if async_task:
    ...
    move_plate(ham_int, reader_site, plate_destination, gripHeight = 12)
    return plate_datas
...
default_liq_class = 'StandardVolumeFilter_Water_DispenseJet_Part'
...
def aspirate_seq(ham_int, sequence, vols, **more_options):
    #assert_parallel_nones(pos_tuples, vols)
    #logging.info('aspirate: Aspirate volumes ' + str(vols) + ' from positions [' +
    #        '; '.join((labware_pos_str(*pt) if pt else '(skip)' for pt in pos_tuples)) +
    #        (']' if not more_options else '] with extra options ' + str(more_options)))
